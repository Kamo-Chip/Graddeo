...
const CompanyCandidateList = () => {
  ...
  const getJobs = async () => {
    ...
    for (let i = 0; i < data.length; i++) {
      let res = await getDoc(doc(db, "candidates", data[i].candidateId));
      data[i].candidateDetails = res.data();
      let candidateToAdd = res.data();

      if (
        !candidateList.some(
          (candidate) => candidate.candidateId == candidateToAdd.candidateId
        )
      ) {
        candidateList.push({ ...candidateToAdd, appliedFor: data[i].jobName });
      } else {
        let idx = candidateList.findIndex(
          (element) => element.candidateId == candidateToAdd.candidateId
        );
        console.log(idx);
        candidateList[idx].appliedFor += `, ${data[i].jobName}`;
        // candidateList[
        //   candidateList.indexOf(candidateToAdd)
        // ].appliedFor += `, ${data[i].jobName}`;
      }
    }
    // console.log(candidateList);
    setCandidates(candidateList);
    setCandidatesToDisplay(candidateList);
    setLoading(false);
  };
...
//     for (let i = 0; i < prospectiveCandidates.length; i++) {
//       let res = await getDoc(
//         doc(db, "candidates", prospectiveCandidates[i].candidateId)
//       );
//       prospectiveCandidates[i].candidateDetails = res.data();
//     }
//     console.log(prospectiveCandidates);

//     setCandidates(prospectiveCandidates);
//   };
...
//                   <span>Applied for {element.jobName}</span>
//                   <CandidateCard candidate={element.candidateDetails} />
//                 </div>
//               );
//             })
//           : null}
